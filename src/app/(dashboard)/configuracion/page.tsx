...
function ConfiguracionContent() {
    ...
    const [activeTab, setActiveTab] = useState<'temporadas' | 'roles'>((tabParam as any) || 'temporadas');
    ...
    return (
        <div className="space-y-8 max-w-6xl mx-auto">
            ...
            <div className="grid gap-8">
                ...
                {activeTab === 'roles' && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                        {/* Search & Assign - Solo visible para Super Admin */}
                        {isSuperadmin && (
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid gap-3">
                                {loadingConfig ? (
                                    <div className="p-8 text-center"><Loader2 className="w-8 h-8 animate-spin mx-auto text-slate-300" /></div>
                                ) : (
                                    hermanos.filter(h => h.rol && h.rol !== 'HERMANO').map((u, i) => (
                                        <div key={i} className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl shadow-sm group">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="font-bold text-slate-900">{u.nombre} {u.apellido1}</p>
                                                    <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">{u.rol}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-4">
                                                ...
                                                {isSuperadmin && u.rol !== 'SUPERADMIN' && (
                                                    <Button
                                                        ...
                                                        onClick={() => handleUpdateRole(u.id, 'HERMANO')}
                                                    >
                                ...
                                {hermanos.filter(h => h.rol && h.rol !== 'HERMANO').length === 0 && !loadingConfig && (
                                    <p className="p-8 text-center text-slate-400 italic bg-white border border-dashed rounded-2xl">No hay otros usuarios con privilegios asignados.</p>
                                )}
                            </div>
                        </div>
                    </div>
